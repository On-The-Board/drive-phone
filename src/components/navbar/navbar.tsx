...
export default function Navbar(props: NavbarProps) {
  ...
  return (
    <nav className="drawer drawer-end p-2 h-12 w-full bg-white flex flex-col fixed top-0 z-30">
      ...
      <div className="drawer-content flex flex-row w-full h-full justify-between items-center overflow-hidden z-[100]">
        ...
        <a href="/" className="uppercase font-extrabold text-xl text-blue-600 mx-auto">
          Drive Phone
        </a>
        <label htmlFor="my-drawer-4"></label>
        {hamburgerActive ? <label htmlFor="my-drawer-4"><Image src={cross} alt="Hamburger Menu" className="h-6 w-6 transition duration-150" onClick={() => setHamburgerActive(false)} /></label> : <label htmlFor="my-drawer-4" className="drawer-button"><Image src={hamburger} alt="Hamburger Menu" className="h-6 w-6 transition duration-150" onClick={() => setHamburgerActive(true)} /></label>}
      </div>
