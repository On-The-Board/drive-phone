...
export default function Comptabilite() {
    ...
    return (
        <main className="px-3 lg:px-36 maxi:px-64 text-black min-h-screen">
                ...
                <div className="flex flex-row gap-5 font-semibold">
                    <select name="" id="" className="bg-white text-lg" onChange={(e) => {
                        if(e.target.value == "month"){
                            setPresta(meetings.filter((e) => parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") < endOfMonth(today) && parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") > startOfMonth(today)))
                        ...
                        if (e.target.value == "today") {
                            setPresta(meetings.filter((e) => parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") < endOfDay(today) && parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") > startOfDay(today)))
                        ...
                        if (e.target.value == "week") {
                            setPresta(meetings.filter((e) => parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") < endOfWeek(today) && parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") > startOfWeek(today)))
                        ...
                        if (e.target.value == "year") {
                            setPresta(meetings.filter((e) => parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") < endOfYear(today) && parseISO(e.date, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") > startOfYear(today)))
            ...
            <h2 className="text-xl font-semibold pt-10 text-start">Réparations</h2>
            <div className="flex flex-col gap-5  Shadow mt-5 rounded-xl lg:p-5 ">
                {presta.map(async (pre) => (
                        <div className="">
                            <div className="border-b border-dashed hidden lg:flex flex-row justify-between">
                            ...
                            <div className=" border-b border-dashed justify-between grid grid-cols-2 lg:hidden">
                                <h2 className="font-semibold">{pre.phoneName}</h2>
                                <p className="text-end"><span className="text-petrole font-semibold w-1/12 text-end">{pre.total}€</span> <br/>{format(parseISO(pre.date), 'dd/MM HH:mm' )}</p>
