            ...
            <div className="w-full px-5 pt-12 lg:pt-20 overflow-hidden text-black flex h-full my-auto">
                <div className="self-center ">
                    <div className="flex flex-col">
                        ...
                        <input type="text" className="bg-white border-b border-b-black" onChange={(e) => {localStorage.setItem("address", e.target.value); setAddress(e.target.value); Identify()}}/>
                    </div>   
                    <div className="flex flex-row gap-5 pt-8">
                        <div className="flex flex-col max-w-[40vw]">
                            <label htmlFor="">Code Postal</label>
                            <input type="number" className="bg-white border-b border-b-black" onChange={(e) => {localStorage.setItem("zipcode", e.target.value); setZip(e.target.value); Identify()}}/>
                        </div>
                        <div className="flex flex-col max-w-[40vw]">
                            <label htmlFor="">Ville</label>
                            <input type="text" className="bg-white border-b border-b-black" onChange={(e) => {localStorage.setItem("city", e.target.value); setTown(e.target.value); Identify()}}/>
                        </div>
                    </div>
                </div>
            </div>
