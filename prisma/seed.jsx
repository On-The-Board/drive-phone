...
async function main() {
    ...
    const data = await gsmarena.catalog.getBrand('huawei-phones-58')

    //Post Devices
    // {data.forEach(async (device) => {
    //     const res = await prisma.device.create({
    //         data: 
    //             {id: device.id, brand_id: 'apple-phones-48', name: device.name, img: device.img, description: device.description },
    //         })
    //         return(res)
    // })}
